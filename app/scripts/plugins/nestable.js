...
;(function($, window, document, undefined)
{
	...
	var defaults = {
			...
			emptyClass      : 'dd-empty',
			expandBtnHTML   : '<i data-action="expand" class="icon-plus-sign-alt"></i>',
			collapseBtnHTML : '<i data-action="collapse" class="icon-minus-sign-alt"></i>',
			group           : 0,
			maxDepth        : 5,
			threshold       : 20,
			dragReverse     : false,
	...
	Plugin.prototype = {
		...
		{
			// RED
			if (li.children(this.options.listNodeName).length && li.has('button').length == 0) {
		...
		dragStart: function(e)
		{
			...
			// RED
			if(this.options.dragReverse) {
				var fix = dragItem.width() - target.outerWidth();
				mouse.offsetX = e.pageX - target.offset().left + fix;
			}
